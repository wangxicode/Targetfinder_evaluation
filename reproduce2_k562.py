...
predictors_df_sorted = pd.concat([predictors_df[0:1977].reset_index().sort_values('promoter_name').set_index(['enhancer_name', 'promoter_name']),predictors_df[1977:41477].reset_index().set_index(['enhancer_name', 'promoter_name'])])

labels_sorted = pd.concat([pd.DataFrame(labels)[0:1977].reset_index().sort_values('promoter_name').set_index(['enhancer_name', 'promoter_name']),pd.DataFrame(labels)[1977:41477].reset_index().set_index(['enhancer_name', 'promoter_name'])])['label']

true_lab_group = pd.DataFrame(labels)[0:1977].groupby(['promoter_name']).size().tolist()

test_fold = []

for i in true_lab_group:
	rnum = randint(0,4)
	while(test_fold.count(rnum) >= 197*2):
		rnum = randint(0,4)
...
for i in range(5):
	neg_lab = neg_lab + [i for j in range(3950*2)]
...
def main(argv = sys.argv):
	...
	for i in range(5):
		...
		if fset=='epw':
			...
			f_fit = predictors_df_sorted.iloc[test_fold != i, [i*3+2 for i in range(136)]]
			f_test = predictors_df_sorted.iloc[test_fold == i, [i*3+2 for i in range(136)]]
		elif fset=='ep':
			f_fit = predictors_df_sorted.iloc[test_fold != i, [i for i in range(408) if (i-2)%3!=0]]
			f_test = predictors_df_sorted.iloc[test_fold == i, [i for i in range(408) if (i-2)%3!=0]]
